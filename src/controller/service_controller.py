...
class ServiceController:
    ...
    def __deploy_service_to_given_ip(self, service_config, deploy_ip):
        tars_service = TarsService(self.config.tars_config.tars_url,
                                   self.config.tars_config.tars_token, self.config.chain_id, deploy_ip, self.config.machine_config.domain_2_ip(deploy_ip))
        # create application
    ...
    def __expand_service_to_given_ip(self, service_config, node_name, expand_node_ip):
        tars_service = TarsService(self.config.tars_config.tars_url,
                                   self.config.tars_config.tars_token, self.config.chain_id, expand_node_ip, self.config.machine_config.domain_2_ip(expand_node_ip))
        # expand the service
        obj_list = service_config.service_obj_list
    ...
    def __undeploy_service(self, service_config):
        for ip in service_config.deploy_ip_list:
            tars_service = TarsService(self.config.tars_config.tars_url,
                                       self.config.tars_config.tars_token, self.config.chain_id, ip, self.config.machine_config.domain_2_ip(ip))
            utilities.log_info(
