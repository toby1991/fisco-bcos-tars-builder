...
class ServiceConfigGenerator:
    ...
    def __generate_gateway_connection_info(self, service_config, ip, is_build_opr):
        ...
        # for nodes.json should use ip @toby
        peers["nodes"] = []
        for peer in service_config.peers:
            (domain, port) = str.split(peer, ":", 2)
            peers["nodes"].append(self.config.machine_config.domain_2_ip(domain) + ":" + port)
